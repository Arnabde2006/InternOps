...
async function routes(fastify) {
  ...
  fastify.post('/validate', {
    schema: { 
      ...
      description: 'Validate certificate fields and clean text with optional AI beautification' 
    },
  }, async (req) => {
  ...
  fastify.post('/generate-achievement', {
    schema: { 
      tags: ['AI Certificates'], 
      description: 'Generate achievement statement using Gemini AI' 
    },
  }, async (req) => {
    const { recipient_name, recognition_type, core_achievement, desired_tone } = req.body;
  ...
  fastify.post('/match-template', {
    schema: { 
      tags: ['AI Certificates'], 
      description: 'AI template matcher with semantic ranking' 
    },
  }, async (req) => {
    const { certificate_type, tone, industry, style, audience, language, user_text } = req.body;
  ...
  fastify.post('/pipeline', {
    schema: { 
      tags: ['AI Certificates'], 
      description: 'Run complete certificate pipeline: validate → generate text → match template' 
    },
  }, async (req) => {
    const {
      name,
  ...
  fastify.post('/bulk-generate', {
    ...
    const { template_id, certificates, send_email, email_subject, email_body } = req.body;
  ...
  fastify.get('/bulk-generate/:jobId', {
    schema: {
      tags: ['AI Certificates'],
  ...
  fastify.post('/tone-customize', {
    schema: {
      tags: ['AI Certificates'],
      description: 'Generate certificate content with specific tone (title, body, closing)'
    },
  }, async (req) => {
    const { recipient_name, company_name, certificate_type, achievement, tone } = req.body;

    if (!AVAILABLE_TONES.includes(tone)) {
      return { success: false, error: `Invalid tone. Choose from: ${AVAILABLE_TONES.join(', ')}` };
    }
  ...
  fastify.post('/generate-multilanguage', {
    schema: {
      tags: ['AI Certificates'],
      description: 'Generate certificate content in specified language'
    },
  }, async (req) => {
    const { recipient_name, company_name, certificate_type, achievement, language } = req.body;

    if (!SUPPORTED_LANGUAGES.includes(language)) {
      return { success: false, error: `Unsupported language. Choose from: ${SUPPORTED_LANGUAGES.join(', ')}` };
    }
  ...
  fastify.post('/preview', {
    ...
    const { recipient_name, title, body, closing, template_name, logo_url } = req.body;
